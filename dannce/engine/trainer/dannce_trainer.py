...
# define a global frame index for saving predicted keypoints
GLOBAL_FRAME_IDX = 0
...
class DannceTrainer(BaseTrainer):
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            result_msg = f"Epoch[{epoch}/{self.epochs}] " + "".join(
                f"train_{k}: {val:.4f} " for k, val in train_stats.items()
            )
            ...
            result_msg = result_msg + "".join(
                f"val_{k}: {val:.4f} " for k, val in valid_stats.items()
            )
    ...
    def _forward(self, epoch, batch, train=True):
        ...
        if train and self.form_batch:
            volumes, grid_centers = form_batch(
                volumes.permute(0, 2, 3, 4, 1), grid_centers, batch_size=self.form_bs
            )
        ...
        inference_path = "/home/jovyan/vast/ckapoor/keypoint-tracking/inference_single_frame_single_instance_vlim150/"
        if not os.path.exists(inference_path):
            os.makedirs(inference_path)

        # save keypoints
        kpt_gt_path = os.path.join(inference_path, f"gt_kpt3d.json")
        kpt_pred_path = os.path.join(inference_path, f"pred_kpt3d.json")

        kpt_gt_np = keypoints_3d_gt.detach().cpu().numpy().tolist()
        kpt_pred_np = keypoints_3d_pred.detach().cpu().numpy().tolist()

        import json

        with open(kpt_gt_path, "w") as f:
            json.dump(kpt_gt_np, f)

        with open(kpt_pred_path, "w") as f:
            json.dump(kpt_pred_np, f)
    ...
    def _train_epoch(self, epoch):
        ...
        epoch_loss_dict, epoch_metric_dict = self._average(
            epoch_loss_dict
