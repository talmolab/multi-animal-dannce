...
class DataGenerator_3Dconv(DataGenerator):
    ...
    def pj_grid(self, X_grid, camname, ID, experimentID):
        ...
        session_idx = ID.split("_")[-1]
        frame_idx = ID.split("_")[1]
        view = camname.split("_")[-1]
        imgdir = "/home/jovyan/vast/ckapoor/keypoint-tracking/slap_2m_sample/frames/"
        frame_name = f"{session_idx}_{frame_idx}_{view}.jpg"
        frame_path = os.path.join(imgdir, frame_name)
        thisim = imageio.imread(frame_path)[
    ...
    def pj_grid_post(self, X_grid, camname, ID, experimentID, com, com_precrop, thisim):
        ...
        t = self.camera_params[experimentID][camname]["t"]
        K = self.camera_params[experimentID][camname]["K"]
        Rdistort = self.camera_params[experimentID][camname]["RDistort"]
        Tdistort = self.camera_params[experimentID][camname]["TDistort"]
